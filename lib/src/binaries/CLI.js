...
commander_1.program
    ...
    .argument('[projectNames...]', 'List of projects to be affected (default: all)')
    .action((branch, projectNames, options) => {
    try {
        projectManager.loadProjectsFromFile();
        projectManager.executeCommandsForProjects('update', projectNames.length === 0 ? Object.keys(projectManager.allProjects) : projectNames, Object.assign(Object.assign({}, options), { branch }));
    }
    catch (error) {
        logger_1.default.err(error);
        process.exit(constants_1.constants.ERROR_EXIT);
