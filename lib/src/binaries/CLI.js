 ...
 */
function getCommandArgs(isNpmCommand) {
    let argsIndex;
    let args = [];
...
program
    ...
    .action((options) => {
    try {
        projectManager.executeCommandsForProjects('exec', projectManager.getProjectNames(options.projects), Object.assign(Object.assign({}, options), { command: [...options.command, ...getCommandArgs(options.command[0] === 'npm')] }));
    }
    catch (error) {
        logger_1.default.err(error);
        process.exit(constants_1.constants.ERROR_EXIT);
