...
class Config {
     ...
     */
    validateSchema(sch, conf, parent = '') {
     ...
     */
    validateProp(schAttr, key, conf, parent = '') {
        const sch = Object.assign({}, schAttr);
        delete sch.default;
        for (const prop in sch) {
            if (!this.hasError(validators_1.Validator.isValid(prop, sch[prop], conf[key]), this._errMsgs[prop], parent, key, sch[prop], typeof conf[key]))
                return;
        }
