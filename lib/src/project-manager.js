...
class ProjectManager {
    constructor() {
        this.CONFIG_FILE_PATH = path_1.default.join(constants_1.constants.CONFIG_FOLDER, constants_1.constants.CONFIG_FILE);
        ...
        this.generateConfigFile = () => {
            ...
            if (this.configFileExists()) {
                ...
                (0, child_process_1.execSync)(`cp ./configs/project-names-example.json ${this.CONFIG_FILE_PATH}`, { stdio: 'inherit' });
        ...
        this.loadProjectsFromFile = () => {
            ...
            const { cwd, user, runnableProjects, noRunnableProjects } = JSON.parse(data);
            runnableProjects.forEach((projectJSON) => {
                this.runnableProjects[projectJSON.name] = new project_1.Project(projectJSON, { cwd, user });
            });
            noRunnableProjects.forEach((projectJSON) => {
                this.noRunnableProjects[projectJSON.name] = new project_1.Project(projectJSON, { cwd, user });
            });
        ...
        this.executeCommandsForProjects = (command, projectNames, options) => {
            ...
            for (const projectName of projectNames) {
                if (!(projectName in projectList))
                    throw new Error(`Project \`${projectName}\` is not specified in \`${this.CONFIG_FILE_PATH}\` file.
